...
describe(`NftContract`, async () => {
  ...
  before(async () => {
    ...
    const defaultGasPrice = await contract.connect(accounts.artist).getGasPrice();
    console.log(`defaultGasPrice`, {
      defaultGasPrice_min_gwei: defaultGasPrice[0].div(1000000).toNumber() / 1000,
      defaultGasPrice_max_gwei: defaultGasPrice[1].div(1000000).toNumber() / 1000,
    });

    const actualGasPrice = (await contract.connect(accounts.artist).setGasPrice(1, 1)).gasPrice;
    if (actualGasPrice) {
      console.log(`actualGasPrice`, {
        ...
        GAS_PRICE_MIN_gwei: GAS_PRICE_MIN.div(1000000).toNumber() / 1000,
        actualGasPrice,
        GAS_PRICE_MAX,
        GAS_PRICE_MIN,
      });
    ...
    const newGasPrice = await contract.connect(accounts.artist).getGasPrice();
    console.log(`newGasPrice`, {
      newGasPrice_min_gwei: newGasPrice[0].div(1000000).toNumber() / 1000,
      newGasPrice_max_gwei: newGasPrice[1].div(1000000).toNumber() / 1000,
      GAS_PRICE_MAX,
      GAS_PRICE_MIN,
    });
  ...
  let GAS_PRICE_MIN = ethers.utils.parseUnits(`10`, 9);
  ...
  const getTokenId = (projectId: number, projectTokenIndex: number) =>
    BigNumber.from(projectId).mul(PROJ_BUCKET_SIZE).add(projectTokenIndex);
  ...
  describe(`metadata`, () => {
    ...
    it(`!onlyArtist: Should FAIL to change baseURI`, async () => {
      await expect(contract.connect(accounts.other1).setBaseURI(`https://test.com/`)).revertedWith(`a`);
    });
  });
  ...
  describe(`project admin`, () => {
    ...
    it(`Should FAIL to create project twice`, async () => {
      ...
      await expect(contract.connect(accounts.artist).createProject(PROJ_ID, RES_COUNT, MINT_PRICE)).revertedWith(`b`);
    });
    ...
    it(`Should FAIL to set project supply below minted`, async () => {
      await contract.connect(accounts.artist).createProject(PROJ_ID, RES_COUNT, MINT_PRICE);
      await expect(contract.connect(accounts.artist).setProjectTokenSupply(PROJ_ID, RES_COUNT)).revertedWith(`s`);
    });
    ...
    it(`Should FAIL to set project supply too large`, async () => {
      await contract.connect(accounts.artist).createProject(PROJ_ID, RES_COUNT, MINT_PRICE);
      await expect(contract.connect(accounts.artist).setProjectTokenSupply(PROJ_ID, PROJ_BUCKET_SIZE + 1)).revertedWith(
        `S`,
      );
    });

    it(`Should FAIL to set project supply - if not created`, async () => {
      await expect(contract.connect(accounts.artist).setProjectTokenSupply(PROJ_ID, 10)).revertedWith(`n`);
    });
  ...
  describe(`minting`, () => {
    ...
    it(`Should FAIL to mint - if owned (mint twice)`, async () => {
      ...
      await expect(contract.connect(accounts.other1).mint(tokenId, { value: MINT_PRICE })).revertedWith(`O`);
    });
    ...
    it(`Should FAIL to mint - if not sequential`, async () => {
      ...
      await expect(contract.connect(accounts.other1).mint(tokenId, { value: MINT_PRICE })).revertedWith(`i`);
    });
    ...
    it(`Should FAIL to mint - if not enough money`, async () => {
      ...
      const tokenId = getTokenId(PROJ_ID, RES_COUNT);
      await expect(contract.connect(accounts.other1).mint(tokenId, { value: MINT_PRICE - 1 })).revertedWith(`$`);
    });
  ...
  describe(`transfers`, () => {
    it(`Should transfer reserved project token`, async () => {
      ...
      await contract
        .connect(accounts.artist)
        .transferFrom(accounts.artist.address, accounts.other1.address, getTokenId(PROJ_ID, RES_COUNT - 1));
    });
    ...
    it(`Should transfer reserved project token (safe)`, async () => {
      ...
      await contract
        ...
        [`safeTransferFrom(address,address,uint256)`](
          ...
          getTokenId(PROJ_ID, RES_COUNT - 1),
        );
    });
    ...
    it(`Should FAIL to transfer unowned token - from self`, async () => {
      ...
      await expect(
        contract
          .connect(accounts.other1)
          .transferFrom(accounts.other1.address, accounts.other2.address, getTokenId(PROJ_ID, RES_COUNT - 1)),
      ).revertedWith(`o`);
    });
    ...
    it(`Should FAIL to transfer to null address`, async () => {
      ...
      await expect(
        contract
          .connect(accounts.artist)
          .transferFrom(accounts.artist.address, ethers.constants.AddressZero, getTokenId(PROJ_ID, RES_COUNT - 1)),
      ).revertedWith(`t`);
    });
  });
